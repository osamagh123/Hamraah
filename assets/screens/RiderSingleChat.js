...
useEffect(() => {
  const ReadRider= async() =>{
    try{
        ...
        .where('status', 'in', ['In Progress','Started','Requested','Pending','Waiting'])
        .get()
        .then((querySnapshot) =>{
  ...
  const renderItem = (item) => {
    ...
    return(
      <View alignItems={status}>
        <Text fontSize='10' mb='2' color='white'>{item.item.name}</Text>
  ...
  return (
    <SafeAreaView>
      <Center bgColor="#252525">
        ...
        <View  mt="5" h={windowHeight - 180 } justifyContent="flex-end">
